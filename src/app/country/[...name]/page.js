  ...
  useEffect(() => {
    fetch(
      ...
      .then((response) =>
        response?.filter(
          (item) =>
            item.name.common === params.name.toString().replace(/-/g, " ")
        )
      )
      .then((response) => {
        ...
        <div className="lg:ml-[120px]">
          ...
          <div className="space-y-8 xl:flex xl:justify-start xl:items-start xl:space-x-[50px] xl:space-y-0">
            <div>
              ...
              <p className="text-[14px] leading-8 font-semibold">
                Region:{" "}
                <span className="font-light">{countryData[0]?.region}</span>
              </p>
              <p className="text-[14px] leading-8 font-semibold">
                Sub Region:{" "}
                <span className="font-light">{countryData[0]?.subregion}</span>
              </p>
              <p className="text-[14px] leading-8 font-semibold">
                Capital:{" "}
                <span className="font-light">{countryData[0]?.capital}</span>
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-[14px] leading-8 font-semibold">
                Languages: &nbsp;
                {countryData[0]
                  ? Object.keys(countryData[0]?.languages).map(
                      (langKey, index) => (
                        <span key={index} className="font-light">
                          {(index ? ", " : "") +
                            countryData[0]?.languages[langKey]}
                        </span>
                      )
                    )
                  : []}
